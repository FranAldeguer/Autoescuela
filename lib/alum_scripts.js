function insertar_modificar(){
    ...
    if(nom == '' || isNaN(telf)){
        ...
        $.post(
            'http://localhost/ProyectoAutoescuela/controles/Control_alumnos.php', 
            {
                inputid           : id, 
                inputdni          : dni, 
                inputnombre       : nom, 
            ...
            function(data){
                ...
                if(id_comporobacion == 0 && data!=0){
                    var lista = ""
                    lista = lista + '<tr id='+data+'>';
                    lista = lista + '<td class="id">'+data+'</td><td class="dni">'+dni+'</td><td class="nombre">'+nom+'</td><td class="apellidos">'+ape+'</td><td class="fecha_nac">'+fnac+'</td><td class="telefono">'+telf+'</td><td class="mail">'+mail+'</td><td class="id_prof">'+id_prof+'</td>';
                    lista = lista + "<td align='center'><img src='img/borrar.jpg' onclick='borrar("+data+", \""+nom+"\", this)' height='25px' width='25px'></td>";
                    lista = lista + "<td align='center'><img src='img/modificar.png' onclick='cargardatos("+data+")' height='25px' width='25px'></td>";
                    lista = lista + '</tr>';
...
function borrar(id, nombre, td){
    ...
    if (ventana) {
        $.post('http://localhost/ProyectoAutoescuela/controles/Control_alumnos.php', {funcion: "borrar_registro", id_borrar: id},
            function (data){});
            removeTR(td);
    }
    else {
