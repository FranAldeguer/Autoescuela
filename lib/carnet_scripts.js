function insertar_modificar(){
    ...
    if(nom == '' || isNaN($("#inputprecio").val())){
        ...
        $.post('http://localhost/Autoescuela/controles/Control_carnets.php', {inputid:id, inputnombre:nom, inputprecio: pre, inputdescripcion:desc},
        function(data){
...
function borrar(id, nombre, td){
    ...
    if (ventana) {
        $.post('http://localhost/Autoescuela/controles/Control_carnets.php', {funcion: "borrar_registro", id_borrar: id},
            function (data){});
            removeTR(td);
    }
    else {
