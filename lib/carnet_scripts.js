function insertar_modificar(){
    ...
    if(nom == '' || isNaN($("#inputprecio").val())){
    	if(nom == ''){
	        alert("El carnet debe llevar un nombre");
	        $('#inputnombre').focus();
    	}
    	if(isNaN(pre)){
    		alert("El precio tiene que ser un numero");
    		$("#inputprecio").focus();
    	}
    }else{
        $.post('http://localhost/ProyectoAutoescuela/controles/Control_carnets.php', {inputid:id, inputnombre:nom, inputprecio: pre, inputdescripcion:desc},
        function(data){
...
function borrar(id, nombre, td){
    ...
    if (ventana) {
        $.post('http://localhost/ProyectoAutoescuela/controles/Control_carnets.php', {funcion: "borrar_registro", id_borrar: id},
            function (data){});
            removeTR(td);
    }
    else {
