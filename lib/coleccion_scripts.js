function insertar_modificar(){
    ...
    if(nom == '' || id_car == 0){
        ...
        $.post('http://localhost/ProyectoAutoescuela/controles/Control_colecciones.php', {inputid:id, inputnombre:nom, selectid_carnet:id_car},
        function(data){
...
function borrar(id, nombre, td){
    ...
    if (ventana) {
        $.post('http://localhost/ProyectoAutoescuela/controles/Control_colecciones.php', {funcion: "borrar_registro", id_borrar: id},
            function (data){});
            removeTR(td);
    }
    else {
