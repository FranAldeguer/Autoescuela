function insertar_modificar(){
    ...
    if(isNaN(cant) || id_alu == 0 || fecha == ""){
        ...
        $.post('http://localhost/ProyectoAutoescuela/controles/Control_entregas.php', {inputid:id, inputfecha:fecha, inputcantidad:cant, selectid_alumno: id_alu},
        function(data){
...
function borrar(id, nombre, td){
    ...
    if (ventana) {
        $.post('http://localhost/ProyectoAutoescuela/controles/Control_entregas.php', {funcion: "borrar_registro", id_borrar: id},
            function (data){});
            removeTR(td);
    }
    else {
