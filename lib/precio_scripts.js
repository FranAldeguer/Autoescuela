function insertar_modificar(){
    ...
    if(isNaN(pre)){
    	...
    	alert('llega');
        $.post('http://localhost/Autoescuela/controles/Control_precios.php', {inputid:id, inputprecio:pre, inputhora:hor},
        function(data){
...
function borrar(id, hora, td){
    ...
    if (ventana) {
        $.post('http://localhost/Autoescuela/controles/Control_precios.php', {funcion: "borrar_registro", id_borrar: id},
            function (data){});
            removeTR(td);
    }
    else {
