function insertar_modificar(){
    ...
    if(nom == ''){
        ...
        $.post('http://localhost/Autoescuela/controles/Control_profesores.php', {inputid:id, inputdni:dni, inputnom:nom, inputape: ape, inputtelf:telf, inputmail:mail, inputpractico:practico, inputencargado:encargado},
        function(data){
            if(telf == ""){telf = 0}
...
function borrar(id, nombre, td){
    ...
    if (ventana) {
        $.post('http://localhost/Autoescuela/controles/Control_profesores.php', {funcion: "borrar_usuario", id_prof: id},
            function (data){});
            removeTR(td);
    }
    else {
