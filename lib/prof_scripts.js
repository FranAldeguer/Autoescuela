function insertar_modificar(){
    ...
    if(nom == '' || isNaN(telf)){
    	if(nom == ''){
	        alert("El profesor debe llevar un nombre");
	        $('#inputnombre').focus();
    	}
    	if(isNaN(telf)){
    		alert("El campo telefono solo admite números");
    		$("#inputtelefono").focus();
    	}
    }else{
        ...
        $.post('http://localhost/ProyectoAutoescuela/controles/Control_profesores.php', {inputid:id, inputdni:dni, inputnombre:nom, inputapellidos: ape, inputtelefono:telf, inputmail:mail, inputpractico:practico, inputencargado:encargado},
        function(data){
            if(telf == ""){telf = 0}
...
function borrar(id, nombre, td){
    ...
    if (ventana) {
        $.post('http://localhost/ProyectoAutoescuela/controles/Control_profesores.php', {funcion: "borrar_registro", id_borrar: id},
            function (data){});
            removeTR(td);
    }
    else {
