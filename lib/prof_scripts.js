function insertar_modificar(){
    ...
    if(nom == '' || isNaN(telf)){
        ...
        $.post('http://localhost/Autoescuela/controles/Control_profesores.php', {inputid:id, inputdni:dni, inputnombre:nom, inputapellidos: ape, inputtelefono:telf, inputmail:mail, inputpractico:practico, inputencargado:encargado},
        function(data){
            if(telf == ""){telf = 0}
...
function borrar(id, nombre, td){
    ...
    if (ventana) {
        $.post('http://localhost/Autoescuela/controles/Control_profesores.php', {funcion: "borrar_registro", id_borrar: id},
            function (data){});
            removeTR(td);
    }
    else {
