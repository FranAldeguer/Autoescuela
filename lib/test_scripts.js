function insertar_modificar(){
    ...
    if(isNaN(num) || isNaN(num_pre) || id_col == 0){
        ...
        $.post('http://localhost/ProyectoAutoescuela/controles/Control_tests.php', {inputid:id, inputnumero:num, inputnum_preguntas: num_pre, selectid_coleccion: id_col},
        function(data){
...
function borrar(id, nombre, td){
    ...
    if (ventana) {
        $.post('http://localhost/ProyectoAutoescuela/controles/Control_tests.php', {funcion: "borrar_registro", id_borrar: id},
            function (data){});
            removeTR(td);
    }
    else {
